...
public class GregAndArray {
    public static void main(String[] args) {
        ...
        // Read the size of the array (n), number of operations (m), and number of queries (k)
        int n = sc.nextInt();
        int m = sc.nextInt();
        int k = sc.nextInt();
        ...
        // Store the operation details: [l, r, d]
        long[][] updates = new long[m][3];
        for (int i = 0; i < m; i++) {
            updates[i][0] = sc.nextLong(); // l
            updates[i][1] = sc.nextLong(); // r
            updates[i][2] = sc.nextLong(); // d
        }

        // Prefix sum array for counting how many times each operation is applied
        long[] s = new long[m + 2];

        // Read the range of queries and update the prefix array `s`
        for (int i = 0; i < k; i++) {
            int x = sc.nextInt(); // start of query
            int y = sc.nextInt(); // end of query
            s[x]++;
            s[y + 1]--;
        }

        // Compute the prefix sums for `s` to determine the actual application count for each operation
        for (int i = 1; i <= m; i++) {
            s[i] += s[i - 1];
        }

        // Second difference array to apply operations efficiently
        long[] add = new long[n + 2];
        for (int i = 0; i < m; i++) {
            add[(int) updates[i][0]] += s[i + 1] * updates[i][2];
            add[(int) updates[i][1] + 1] -= s[i + 1] * updates[i][2];
        }

        // Compute the final prefix sums in `add` and apply them to the original array
        for (int i = 1; i <= n; i++) {
            add[i] += add[i - 1];
